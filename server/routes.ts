...
async function setSession(res: any, userId: number): Promise<string> {
  ...
  try {
    // Persist to DB for Serverless support
  ...
  // Optimized Cookie for Custom Domains (Lax is safer for redirects)
...
export async function registerRoutes(
  ...
  app.patch(
    ...
    async (req, res) => {
      const u = await storage.updateUserRole(
        Number(req.params.id),
  ...
  app.patch(
    ...
    async (req, res) => {
      const u = await storage.suspendUser(
        Number(req.params.id),
  ...
  app.patch("/api/admin/settings", requireRole("admin"), async (req, res) => {
    const s = await storage.upsertSiteSetting(
      req.body.key,
